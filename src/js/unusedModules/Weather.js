define(['promise!config','promise!runtime ','Renderer','Storage'], function Weather(storage,config,runtime,renderer) {
    var self = {};
    ...
    self.buildURL = function(city, lat, lng, days, units){
        days = days || 5;
        units = units || 'metric';
    ...
    self.mainWeatherIconChar = function(icon){
        /*
    ...
    self.doGet = function(city, latitude, longitude, days, cc, done){
    ...
    self.get = function(params, done){
            if(!config.config.with_weather) return done && done(true);


            var oldPeriod = this.oldPeriod;
            var city = params.city || self.runtime.location.city.short_name;
            var cc = params.cc || self.runtime.location.country.short_name;
